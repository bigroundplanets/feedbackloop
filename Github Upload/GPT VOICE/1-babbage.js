...
// Structure of how to communicate with OpenAI API
let options = {
  method: "POST",
  headers: {
...
function getText() {
  ...
  options.body = JSON.stringify({
    model: "babbage", // Specify the model to use
    prompt: inputValue, // Use the input value as the prompt
  ...
  fetch(url, options)
    ...
    .then(function(response) {
      if (response.choices && response.choices[0]) { // Check if there are choices in the response
        var assistantMessage = response.choices[0].text; // Get the model's response
        
        // Display the response
        console.log("response:", assistantMessage); // Log the response
