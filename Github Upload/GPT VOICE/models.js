...
function getText() {
  ...
  options.body = JSON.stringify({
    model: "gpt-3.5-turbo-0301", // Specify the model to use
    messages: [
      {
        "role": "system",
        "content": "respond"
    ...
    temperature: 0.8 // Set the temperature for randomness
  });
  ...
  fetch(url, options)
    ...
    .then(function(response) {
      if (response.choices && response.choices[0]) { // Check if there are choices in the response
        ...
        // Speak the response
        speech.speak(assistantMessage); // Say the assistant's message
      }
    });
}
